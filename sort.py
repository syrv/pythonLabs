...
def BubbleSort(A):                  # сортировка пузырьком
    for i in range(len(A)):
        for j in range(len(A)-1-i):
            if A[j] > A[j+1]:
                a = A[j]    # the same A[j + 1], A[j] = A[j], A[j + 1]
                A[j] = A[j+1]
                A[j+1] = a


def Insert(A):                        ## сортировка  Insert (меняем на название в вызове)
...
def Shaker(A):                    ## сортировка  Shaker  (меняем на название в вызове)
    for i in range(len(A) // 2):  ## делим список на 2 используя целочисленное деление
        for j in range(len(A) - 1 - i): ## цикл для левой части
            if A[j] > A[j + 1]:   # как и bubblesort с методом "3 стаканов"
                a = A[j]     # the same  A[j + 1], A[j] = A[j], A[j + 1]
                A[j] = A[j + 1]
                A[j + 1] = a
        for j in range(len(A) - 2 - i, i + 1, -1): #  цикл для правой
            if A[j] < A[j - 1]:   # -/-
                a = A[j]
                A[j] = A[j - 1]
                A[j - 1] = a
...
def Select(A):                    ## сортировка Select   (меняем на название в вызове)
    for i in range(len(A)-1):
        m=i                        # для мин значения
        for j in range(i, len(A)):
            if A[j]<A[m]:
                m=j
        a = A[m]  # the same # A[i], A[m] = A[m], A[i]
        A[m] = A[i]
        A[i] = a
